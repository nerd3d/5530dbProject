	...
	public static void viewTopUseful(String driver) 
	{
		...
		try {
			...
			if(n < 0) 
				...
				String query = "SELECT login, date, rating, text, fid, avgRating FROM Feedback F, (SELECT vin, AVG(rating) avgRating FROM Feedback GROUP BY vin) AF, (SELECT vin FROM Owns WHERE login = '" +driver+"') V  WHERE F.vin = V.vin AND AF.vin = F.vin ORDER BY avgRating desc LIMIT " +n+ ";";	
				//SELECT login, date, rating, text, fid, avgRating FROM Feedback F, (SELECT vin, AVG(rating) avgRating FROM Feedback GROUP BY vin) AF, (SELECT vin FROM Owns WHERE login = 'flash') V  WHERE F.vin = V.vin AND AF.vin = F.vin ORDER BY avgRating desc LIMIT 2;
				viewAndMenu(query);
			}
			
		}catch(Exception e){System.out.println("Invalid input. Requires numerical entry (example: 3)");}
